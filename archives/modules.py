...
class UNet(nn.Module):
    def __init__(self, c_in=3, c_out=3, size = 64, time_dim=256, device="cuda"):
        ...
        self.down1 = Down(size, size2) # (64, 128)
        self.sa1 = SelfAttention(size2, size_2) # (128, 32)
        self.down2 = Down(size2, size4) # (128, 256)
        self.sa2 = SelfAttention(size4, size_4) # (256, 16)
        self.down3 = Down(size4, size4) # (256, 256)
...
class UNet_conditional(nn.Module):
    def __init__(self, c_in=3, c_out=3, time_dim=256, num_classes=None, device="cuda", USE_GPU = True):
        super().__init__()
        self.device = device
        self.time_dim = time_dim
